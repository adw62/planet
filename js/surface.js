...
export function renderSurfaceTextures(hmap, himap, visual) {
  ...
  const col = makeComposite(visual);
  ...
  for (let i=0;i<S*S;i++){
    ...
    const [r,g,b]=col(hmap[i], himap[i], lat);
  ...
  if (moltenGlow > 0.01 || (civ > 0.01 && seeds.length)) {
    ...
    const reach = (0.03 + civ * 0.17) * S;                 // metro radius in pixels
    const reach2 = reach * reach, halfS = S / 2;
    const doCities = civ > 0.01 && seeds.length;
    for (let i=0;i<S*S;i++){
      let r=0,g=0,b=0;
  ...
  return { map:new THREE.CanvasTexture(dc), emissiveMap, specularMap };
}
